...
extension StageViewOverlaySlots {
	...
	public func toAnchoredCollection() -> ViewportOverlayCollection {
		...
		if let topLeading {
			collection = collection.hostAccessory(
				anchor: .topLeading,
				content: UncheckedAnyViewBox(topLeading)
			)
		}
		if let top {
			collection = collection.hostAccessory(
				anchor: .top,
				content: UncheckedAnyViewBox(top)
			)
		}
		if let topTrailing {
			collection = collection.hostAccessory(
				anchor: .topTrailing,
				content: UncheckedAnyViewBox(topTrailing)
			)
		}
		if let bottomLeading {
			collection = collection.hostAccessory(
				anchor: .bottomLeading,
				content: UncheckedAnyViewBox(bottomLeading)
			)
		}
		if let bottom {
			collection = collection.hostAccessory(
				anchor: .bottom,
				content: UncheckedAnyViewBox(bottom)
			)
		}
		if let bottomTrailing {
			collection = collection.hostAccessory(
				anchor: .bottomTrailing,
				content: UncheckedAnyViewBox(bottomTrailing)
			)
		}
...
@available(
	*,
	deprecated,
	message: "Use ViewportOverlayCollection with anchored positioning"
)
public struct StageViewOverlaySlots {
	...
	public func bottom<Content: View>(@ViewBuilder _ content: () -> Content)
		-> Self
	{
		var copy = self
	...
	public func bottomLeading<Content: View>(
	...
	public func bottomTrailing<Content: View>(
		@ViewBuilder _ content: () -> Content
	) -> Self {
		var copy = self
	...
	public func topLeading<Content: View>(@ViewBuilder _ content: () -> Content)
		-> Self
	{
		var copy = self
	...
	public func topTrailing<Content: View>(@ViewBuilder _ content: () -> Content)
		-> Self
	{
		var copy = self
...
@available(
	*,
	deprecated,
	message: "Use ViewportOverlayContainer with anchored positioning"
)
