...
@Suite("Legacy Slot-to-Anchored Migration")
struct LegacyMigrationTests {

    @Test
    func toAnchoredCollectionPreservesTopLeadingContent() {
        let slots = StageViewOverlaySlots.empty.topLeading {
            Text("Badge")
        }

        let collection = slots.toAnchoredCollection()
        let grouped = collection.groupedByAnchor()
        let items = grouped[.topLeading] ?? []

        #expect(items.count == 1)
        #expect(items[0].role == .hostAccessory)
        #expect(items[0].content != nil)
    }

    @Test
    func toAnchoredCollectionPreservesTopContent() {
    ...
    @Test
    func toAnchoredCollectionPreservesTopTrailingContent() {
        let slots = StageViewOverlaySlots.empty.topTrailing {
            Text("Menu")
        }
    ...
    @Test
    func toAnchoredCollectionPreservesBottomLeadingContent() {
        let slots = StageViewOverlaySlots.empty.bottomLeading {
            Text("Gizmo")
        }
    ...
    @Test
    func toAnchoredCollectionPreservesBottomContent() {
        let slots = StageViewOverlaySlots.empty.bottom {
            Text("Status")
        }

        let collection = slots.toAnchoredCollection()
        let grouped = collection.groupedByAnchor()
        let items = grouped[.bottom] ?? []
    ...
    @Test
    func toAnchoredCollectionPreservesBottomTrailingContent() {
        let slots = StageViewOverlaySlots.empty.bottomTrailing {
            Text("Picker")
        }
    ...
    @Test
    func toAnchoredCollectionPreservesMultipleSlots() {
        let slots = StageViewOverlaySlots.empty
            .topLeading { Text("Badge") }
            .top { Text("Toolbar") }
            .topTrailing { Text("Menu") }

        let collection = slots.toAnchoredCollection()
        let grouped = collection.groupedByAnchor()

        #expect(grouped[.topLeading]?.count == 1)
        #expect(grouped[.top]?.count == 1)
        #expect(grouped[.topTrailing]?.count == 1)
    }

    @Test
    func emptySlotsConvertToEmptyCollection() {
        let slots = StageViewOverlaySlots.empty
        let collection = slots.toAnchoredCollection()
